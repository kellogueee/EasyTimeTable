...
namespace EasyTimeTable
{
    ...
    public partial class NightAndWeekendPage : ContentPage
    {
            ...
            for (var row = 0; row < 12; row++)
            {
                for (var col = 0; col < 3; col++)
                {
                    ...
                    if (col == 0)
                    {
                        var label = new Label
                        {
                            Text = string.Format("{0:00}", (row + 20) % 24),
                    ...
                    NightAndWeekendBody.Children.Add(stack);
                }
            }
        }
        ...
        protected async override void OnAppearing()
        {
            ...
            //ClassID는 DB ID임
            var items = NightAndWeekendBody.Children.Where(x => x.StyleId == "scheduleStack");
            ...
            if (items.Count() > 0)
            {
                ...
                if (removalSchedule.Count() > 0)
                {
                    ...
                    {
                        NightAndWeekendBody.Children.Remove(item);
                    }
                ...
                foreach (var item in UpdatedSchedule)
                {
                    ...
                    var row = item.StartHour - 20;
                    if (row < 0)
                    {
                        row += 24;
                    }

                    int end = item.EndHour - 20;
